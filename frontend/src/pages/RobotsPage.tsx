...
export default function RobotsPage() {
  ...
  async function handleSave(values: FormValues) {
    try {
      if (editing) {
        await updateRobot(editing.id, { status: values.status, machineInfo: values.machineInfo || null });
      } else {
        await createRobot({ name: values.name, machineInfo: values.machineInfo || undefined });
      }
      closeModal();
      await load(search);
    } catch (e: any) {
