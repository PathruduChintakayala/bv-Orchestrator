...
function Header() {
  ...
  const items = useMemo(() => ([
    ...
    { label: 'Automations', href: '#/automations', match: (h: string) => h.startsWith('#/automations') || h.startsWith('#/processes') || h.startsWith('#/jobs') || h.startsWith('#/triggers') || h.startsWith('#/logs') },
    ...
    <header className="app-header" style={{ position: 'fixed', top: 0, left: 0, right: 0, backgroundColor: '#ffffff', boxShadow: '0 1px 6px rgba(0,0,0,0.06)', display: 'flex', flexDirection: 'column', zIndex: 10 }}>
      ...
      <nav aria-label="Primary" style={{ borderTop: '1px solid #eef2f7', borderBottom: '1px solid #eef2f7', padding: '8px 16px' }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 16 }}>
          {items.filter(it => hasPermission(user, permissions, (it as any).perm)).map(it => {
...
function hasPermission(
  ...
  perm?: { artifact: string; op: 'view' | 'edit' | 'create' | 'delete' },
) {
  if (!perm) return true
